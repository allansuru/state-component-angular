...
export class StoreService extends ComponentStore<ParkingState> {
  ...
  readonly removeCars = this.updater((state: ParkingState, cars: Car[]) => {
    return {
      ...state,
      error: '',
      cars: [...cars],
  ...
  readonly removeCarToParkingLot = this.effect((plate$: Observable<string>) =>
    plate$.pipe(
      concatMap((plate: string) => {
        this.setLoading(true);
        return this.parkingLotService.remove(plate).pipe(
          tap({
            next: (car) => this.removeCars(car),
            error: (e) => this.updateError(e),
          }),
          finalize(() => {
            this.setLoading(false)
